...
const EmployeeDetails = ({navigation, route}) => {
  ...
  const [image, setimage] = useState(route.params.item.profile?route.params.item.profile:'')
    ...
    <SafeAreaView style={{flex: 1}}>
      ...
      <View style={styles.topSection}>
        ...
        activeOpacity={0.8}>
          ...
          //   {uri:image}
          // }
         source={image===''?                       require('../../assets/icons/profile/profile.png')
         :
       image===route.params.item.profile? {uri:`http://18.159.82.242/v1/uploads/${image}`}:
         {uri: image}}
      ...
      <View style={{marginTop: '5%', marginHorizontal:Platform.OS==='android'? '10%':'0%',marginRight:Platform.OS==='ios'?'65%':'0%'}}>
        <Text
          style={{
            fontSize: RFValue(16),
            fontFamily: 'IBMPlexSansHebrew-Bold',
        ...
        <FlatList
          ...
          renderItem={({item, index}) => {
            return (
              <TouchableOpacity
                onPress={() => {
                  navigation.navigate('showService', {item: isEnabled,idimport,image,employeeName});
                }}
                style={{padding: '2%'}}>
                <View
                  style={{
